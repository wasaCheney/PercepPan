...
def get_ckpts_path(opt,
    ...
    # prefixed
    prefix = opt['name']
    # all case XXX we need one more, spsf_reduced model but full dataset
    paradigms = ['spsf_reduced', 'spuf_reduced', 'spuf_full']
    ...
    for paradigm in paradigms:
        if paradigm == 'spuf_reduced':
            continue
        for init in inits:
            if init != 'esrgan':
                continue
            for alpha, beta, gamma in ABGs:
                if (alpha, beta, gamma) == (1, 1, 0.01):
                    lrs = [all_lrs[3]]
                else:
                    continue
                    lrs = all_lrs[2:]
...
def valid(opt, val_loader, model, logger):
...
def test(opt, logger, test_loader, model):
    """For every test set"""
    ...
    scores = valid(opt, test_loader, model, logger)
    return scores
...
def test_flow():
    ...
    for setting, ckpt_dict in result_dict.items():
        ...
        # XXX
        opt['datasets']['val']['dataroot_HR'] = opt['datasets']['val']['dataroot_LR'] = f"data/opticalRSI/{opt['datasets']['val']['name']}/{opt['dataset_type']}/"
