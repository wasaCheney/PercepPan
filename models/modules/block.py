...
class ResidualDenseBlock_5C(nn.Module):
    '''
    Residual Dense Block (shape-keeping operation)
    style: 5 convs
    The core module of Residual Dense Network for Image Super-Resolution,
    CVPR 18
    '''
    ...
    def forward(self, x):
        # Every dense connection: concatenate
        x1 = self.conv1(x)
        x2 = self.conv2(torch.cat((x, x1), 1))
        x3 = self.conv3(torch.cat((x, x1, x2), 1))
        x4 = self.conv4(torch.cat((x, x1, x2, x3), 1))
        x5 = self.conv5(torch.cat((x, x1, x2, x3, x4), 1))
        return x5.mul(0.2) + x  # Residual connection: weighted add
