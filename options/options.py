...
def parse(opt_path, is_train=True):
    ...
    for phase, dataset in opt['datasets'].items():
        ...
        if 'dataroot_HR' in dataset and dataset['dataroot_HR'] is not None:
            dataset['dataroot_HR'] = os.path.join(DRIVE_ROOT,
                                                  dataset['dataroot_HR'])
            if '.lmdb' in dataset['dataroot_HR']:
                is_lmdb = True
        if 'dataroot_HR_bg' in dataset and dataset[
                'dataroot_HR_bg'] is not None:
            dataset['dataroot_HR_bg'] = os.path.join(DRIVE_ROOT,
                                                     dataset['dataroot_HR_bg'])
        if 'dataroot_LR' in dataset and dataset['dataroot_LR'] is not None:
            dataset['dataroot_LR'] = os.path.join(DRIVE_ROOT,
                                                  dataset['dataroot_LR'])
            if '.lmdb' in dataset['dataroot_LR']:
                is_lmdb = True
        dataset['data_type'] = 'lmdb' if is_lmdb else 'img'

        if phase == 'train' and 'subset_file' in dataset and dataset[
                'subset_file'] is not None:
            dataset['subset_file'] = os.path.join(DRIVE_ROOT,
                                                  dataset['subset_file'])

    # path
    for key, path in opt['path'].items():
        if path and key in opt['path']:
            opt['path'][key] = os.path.join(DRIVE_ROOT, path)
    if is_train:
        ...
        results_root = os.path.join(opt['path']['root'], 'results',
                                    opt['name'] + opt['datasets']['val']['name'])
